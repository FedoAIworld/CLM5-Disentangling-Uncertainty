...
def plot_ensemble_spread_mean(site_realizations, observed_df, merged_df, observed_time_ranges, pert_factor, var, label, unit, save_dir, sites, site_full, var_title, save_performance_dir):
    ...
    for site, site_full in zip(sites, site_full):
        ...
        site_result_df = merged_df[merged_df['datetime'].isin(observed_time_ranges[site])]
        #site_result_df = site_result_df[[f'{site}_min', f'{site}_max', f'{site}_{var}']]
...
def create_seasons_and_plot(obs_ens_merged, label, unit, save_dir, sites, site_full, obs_time_ranges):
    # Create the directory if it does not exist
    os.makedirs(save_dir, exist_ok=True)
    ...
    for site, site_name in zip(sites, site_full):
        # Add a 'season' column to the obs_data DataFrame and group by month
        obs_ens_merged['season'] = ((obs_ens_merged['datetime'].dt.month % 12 + 3) // 3).map({1: 'DJF', 2: 'MAM', 3: 'JJA', 4: 'SON'})
        
        # Filter data based on the observed time range
        observed_range = obs_time_ranges[site]
        site_data = obs_ens_merged[(obs_ens_merged['datetime'].isin(observed_range)) & (obs_ens_merged['season'].isin(seasons))]

        
        # Skip sites with no observed data
        if site_data.empty:
            continue
        ...
        # Remove rows with NaN values in observed data
        site_data = site_data.dropna(subset=[obs_col], how='all')

        # Filter ensemble data based on observed data
        ensemble_mean_filtered = site_data[[obs_col, ens_col, 'datetime', 'season']]
        

        obs_box_data = []
        ens_box_data = []

        # Iterate over the seasons and calculate the average variable for each year
        for season in seasons:
            season_data = ensemble_mean_filtered[ensemble_mean_filtered['season'] == season]

            obs_yearly_avg = season_data.groupby(season_data['datetime'].dt.year)[obs_col].mean()
            ens_yearly_avg = season_data.groupby(season_data['datetime'].dt.year)[ens_col].mean()

            obs_box_data.append(obs_yearly_avg.tolist())
            ens_box_data.append(ens_yearly_avg.tolist())

            # Store calculated averages in the list
            for year, obs_avg, ens_avg in zip(obs_yearly_avg.index, obs_yearly_avg, ens_yearly_avg):
                averages_data.append([site_name, year, season, obs_avg, ens_avg])

        # Create a new figure and axis for each site
        #plt.rcParams.update(font)
        fig, ax = plt.subplots(figsize=(15, 12))

        bp_obs = ax.boxplot(obs_box_data, positions=positions - 0.2, patch_artist=True)
        bp_ens = ax.boxplot(ens_box_data, positions=positions + 0.2, patch_artist=True)

        for box in bp_obs['boxes']:
            box.set(facecolor='black')

        for box in bp_ens['boxes']:
            box.set(facecolor='#f03b20')

        ax.set_ylabel(f'{label} {unit}')
        ax.set_title(f'Seasonal {label} - {site_name}')

        ax.set_xticks(positions)
        ax.set_xticklabels(['Spring', 'Summer', 'Fall', 'Winter'])

        # Add legend for each site
        ax.legend([bp_obs['boxes'][0], bp_ens['boxes'][0]], ['Observed', 'Ensemble'])
        ...
        plot_filename = os.path.join(plot_subdir, f'{site.lower()}_seasonal.png')
        plt.savefig(plot_filename)
        plt.close()

    # Create a DataFrame from the averages data
    averages_df = pd.DataFrame(averages_data, columns=['Site', 'Year', 'Season', 'Observed', 'Ensemble'])

    # Create a directory for the CSV file if it does not exist
    csv_dir = os.path.join(save_dir, 'averages_csv')
    os.makedirs(csv_dir, exist_ok=True)

    # Save the DataFrame as a CSV file
    csv_filename = os.path.join(csv_dir, 'seasonal_averages.csv')
